...

{
    ...
    class DataGridAxis extends HTMLElement {
        ...
        valueList() {
            ...
            return new Promise((resolve, reject) => database.readTransaction(
                transaction => transaction.executeSql(statement, [], (_tx, { rows }) => {
                    ...
                    const values = Array.from(rows, ({ value }) => value);
    ...
    class DataGrid extends HTMLElement {
        ...
        schema() {
            ...
            return new Promise((resolve, reject) => database.readTransaction(
                transaction => transaction.executeSql(checkExistence, [name], (_tx, { rows }) => {
                    const [row] = rows;
                    resolve(row && row.sql);
                }),
                reject,
        ...
        initialize() {
            ...
            if (!database) {
                database = openDatabase(
                    ...
                    this.getAttribute('data-db-version'),
                    this.getAttribute('data-db-displayName') || this.getAttribute('data-db-name'),
                    parseInt(this.getAttribute('data-db-size'), 10) || 1024 * 1024,
                );
                this.database = database;
            }

            return this.schema().then((sql) => {
                if (!sql) {
                    return this;
                }
        ...
        refresh() {
            ...
            return this.tableExists()
                .then(exists => exists && this.pivotTable({ colAttrs, rowAttrs, aggregator, filters }))
                .then((table) => {
                    if (!table) {
                        return this;
                    }
        ...
        tableExists() {
            return this.schema().then(sql => !!sql);
        }

        bulkInsert(columns, rows) {
            const { name, database } = this;
            ...
            const callback = (tx) => {
                ...
                const { length } = batch;
                if (length) {
                    tx.executeSql(`${insert}${next.repeat(length - 1)}`, batch.flat(), callback);
                }
            };

            return new Promise((resolve, reject) => database.transaction(callback, reject, resolve));
        };    
        ...
        pivotTable({ rowAttrs, colAttrs, aggregator, filters }) {
            ...
            return new Promise((resolve, reject) => database.readTransaction(
                transaction => transaction.executeSql(selectStatement, [], (tx, { rows }) => {
                    const rowKeys = [];
                    const colKeys = [];
                    const values = [];
                    const { length } = rows;
                    for (let i = 0; i < length; i += 1) {
                        ...
                        const rowKey = rowAttrs.length ? rowAttrs.map(attr => row[attr]) : ['Totals'];
                        if (!rowKeys.length || indexedDB.cmp(rowKey, rowKeys[rowKeys.length - 1])) {
                            rowKeys.push(rowKey);
                            values.push([]);
                        }
                        const colKey = colAttrs.length ? colAttrs.map(attr => row[attr]) : ['Totals'];
                        ...
                        values[rowKeys.length - 1][colIdx] = row.value;
                    }
